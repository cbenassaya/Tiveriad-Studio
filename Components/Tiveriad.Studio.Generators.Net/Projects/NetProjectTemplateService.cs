...
public class NetProjectTemplateService : IProjectTemplateService<InternalType, ProjectDefinition>
{
    ...
    {
        ArgumentNullException.ThrowIfNull(projectDefinitionTemplate,
            "Project Template is null ! Please load it before");
    ...
    {
        var references = _projectDefinitionTemplate.ComponentDefinitions.Where(x => x.Layer == projectDefinition.Layer)
            .SelectMany(x => x.References.Select(y => y.Name)).ToList();
    ...
    public IEnumerable<Dependency> GetDependencies(ProjectDefinition projectDefinition)
    {
        ...
        return _projectDefinitionTemplate.ComponentDefinitions.Where(x => x.Layer == projectDefinition.Layer)
    ...
    public string GetItemPath(InternalType internalType)
    {
        ...
        var classifier = internalType.Reference;
        ...
        var items = _projectDefinitionTemplate.ComponentDefinitions.SelectMany(
            component => component.ComponentItems,
            (component, componentItem) => new { Component = component, ComponentItem = componentItem }
        ).Where(x => x.ComponentItem.Stereotype == internalType.Stereotype).ToArray();
        if (items.Length > 1)
            throw new ArgumentException(
                $"More than one definition for {internalType.Stereotype} stereotype");
        var item = items.FirstOrDefault();

        ArgumentNullException.ThrowIfNull(item,
            $"No definition for {internalType.Stereotype} stereotype");
        var @namespace = internalType.Namespace;
        var subNamespace =
            @namespace.Substring($"{string.Format(item.Component.NamePattern, project.RootNamespace, module.Name)}."
                .Length);
